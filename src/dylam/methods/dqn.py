...
class DQN(nn.Module):
    def __init__(self, args, observation_space, action_space):
        ...
        if len(observation_space.shape) == 3:
            self.q_network = MiniGridQNetwork(
                observation_space,
                num_actions=0,
...
class DQ(DQN):
    def __init__(self, args, observation_space, action_space):
        super(DQ, self).__init__(args, observation_space, action_space)
    ...
    def get_q_value_from_components(self, observation, nets=None):
        _ = torch.Tensor([]).to(self.device)
        components_values = torch.zeros(
            self.num_rewards, observation.shape[0], self.action_size
        ).to(self.device)
        for i in range(self.num_rewards):
            net = self.q_networks[i] if nets is None else nets[i]
            q_values = net(observation, _, _)
...
class UDC(DQ):
    def update_q(
        ...
        _ = torch.Tensor([]).to(self.device)
        qf_losses = []
        next_q = self.get_q_value_from_components(
            next_state_batch, self.target_q_networks
        )
        next_actions = next_q.max(dim=1)[1].unsqueeze(1)
        for i, (q_net, target_q_net) in enumerate(
            zip(self.q_networks, self.target_q_networks)
        ):
            with torch.no_grad():
                target_q_values = target_q_net(next_state_batch, _, _)
                target_q_values = target_q_values.gather(1, next_actions).squeeze()
                target_q_values[done_batch] = 0
                next_q_values = reward_batch[:, i] + self.gamma * target_q_values
            q_values = q_net(state_batch, _, _)
            q_values = q_values.gather(1, action_batch).squeeze()
