...
class ContinuousMountainCar(Continuous_MountainCarEnv, EzPickle):
    """
    ...
    def __init__(
        ...
        self.reward_space = spaces.Box(
            low=np.array([-1.0, -1.0]), high=np.array([0.0, 0.0]), dtype=np.float32
        )
        self.reward_dim = 2
    ...
    def step(self, action: np.ndarray):
        ...
        self.cumulative_reward_info["reward_Fuel"] += reward[1]
        ...
        if self.render_mode == "human":
            self.render()
        return self.state, reward, terminated, False, self.cumulative_reward_info
